import * as React from "react";
const SvgIconExclamationCircle = (props) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    {...props}
  >
    <path
      fill="#000"
      d="m12.02 14.5.102.005a1 1 0 0 1-.101 1.995 1.005 1.005 0 0 1-1.006-1c0-.552.443-1 .995-1zM12 6.75a.75.75 0 0 1 .75.75v4.571a.75.75 0 0 1-1.5 0V7.5a.75.75 0 0 1 .75-.75"
    />
    <path
      fill="#000"
      fillRule="evenodd"
      d="M12 1.25c5.928 0 10.75 4.822 10.75 10.75S17.928 22.75 12 22.75 1.25 17.928 1.25 12 6.072 1.25 12 1.25m0 1.5c-5.101 0-9.25 4.149-9.25 9.25s4.149 9.25 9.25 9.25 9.25-4.149 9.25-9.25S17.101 2.75 12 2.75"
      clipRule="evenodd"
    />
  </svg>
);
export default SvgIconExclamationCircle;

import * as React from "react";
const SvgIconAngleRightSquare = (props) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    {...props}
  >
    <path
      fill="#000"
      d="M10.857 8.263a.76.76 0 0 1 .674.206l3 3a.75.75 0 0 1 0 1.062l-3 3a.75.75 0 0 1-1.061 0 .75.75 0 0 1 0-1.061L12.94 12l-2.47-2.47a.75.75 0 0 1 .387-1.267"
    />
    <path
      fill="#000"
      fillRule="evenodd"
      d="M17.625 2.25c2.66 0 4.125 1.465 4.125 4.125v11.25c0 2.66-1.465 4.125-4.125 4.125H6.375c-2.66 0-4.125-1.465-4.125-4.125V6.375c0-2.66 1.465-4.125 4.125-4.125zm-11.25 1.5c-1.84 0-2.625.785-2.625 2.625v11.25c0 1.84.785 2.625 2.625 2.625h11.25c1.84 0 2.625-.785 2.625-2.625V6.375c0-1.84-.785-2.625-2.625-2.625z"
      clipRule="evenodd"
    />
  </svg>
);
export default SvgIconAngleRightSquare;
